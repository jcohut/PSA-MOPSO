...
mopsopsa_v2 <- function(fn,gn,varcnt,fncnt,lowerbound,upperbound,opt,popsize,maxgen,archivesize,verbosity,pMut,w1,w2,c1,c2,vclamp,npsa) UseMethod("mopsopsa_v2")
mopsopsa_v2<- function(fn, gn=NULL,varcnt,fncnt,lowerbound,upperbound,opt,popsize=100,maxgen=300,archivesize=250,verbosity=0,pMut=0.5,w1=0.9,w2=0.4,c1=1.494,c2=1.494,vclamp=0.5,npsa=5)
{
...
update_archive <- function() {
  ...
  if(leninspop != 0){
    if (ndomCtr + leninspop <= archivesize) {
      ...
      d <- as.matrix(dist(A))
      diag(d) = Inf
      while(length(irepr) < archivesize) {
          for(i in 1:dim(d)[2]) {
              irepr = c(irepr, which.min(d[,i])[1])
              d[,i][which.min(d[,i])] = Inf
          }
      }
      irepr = irepr[1:archivesize]
      archiveVar[1:archivesize,] <<- rbind(archiveVar[1:ndomCtr,], popVar[ixinspop,])[irepr,]
